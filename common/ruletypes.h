...
RULE_INT ( Character, LightBlueModifier, 30 ) 
RULE_INT ( Character, BlueModifier, 90 )
RULE_INT ( Character, WhiteModifier, 100 )
RULE_INT ( Character, YellowModifier, 120 )
RULE_INT ( Character, RedModifier, 140 ) 
...
RULE_BOOL ( Zone, PetZone, true ) // True = pets will zone with player, False = Pets will not zone with player
RULE_CATEGORY_END()
...
RULE_INT ( Combat, HitCapPre30, 39) // live has no cap past 30, should be 29 - cap in attack.cpp  - Kegz @ EpicEmu.com
RULE_INT ( Combat, HitCapPre20, 29) // live has it capped at 40 for whatever dumb reason... this is mainly for custom servers
RULE_INT ( Combat, HitCapPre10, 19) // live has it capped at 20, see above :p
RULE_REAL ( Combat, ACDR, 0) //Set to reduce damage using AC*(this value/100) at 1 it will add 1 damage reduction per 100AC - Kegz @ EpicEmu.com
RULE_CATEGORY_END()
...
RULE_INT ( NPC, EngagedHeadingUpdateTicCount, 20) // Number of NPC::Process() executions before engaged mob will check to FaceTarget()
RULE_CATEGORY_END()
