...
// new hatelist code including damage shield
void HateList::Add(Mob *ent, sint32 in_hate, sint32 in_dam, bool bFrenzy, bool iAddIfNotExist, bool fromDS)
{
   if(!ent)
           return;
   ...
   if (p)
   {
      p->damage += fromDS ? 0 : ((in_dam>=0)?in_dam:0);
      p->ds_damage += fromDS ? ((in_dam>=0)?in_dam:0) : 0;
      p->hate+=in_hate;
      p->bFrenzy = bFrenzy;
   }
