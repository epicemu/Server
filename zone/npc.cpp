	...
	qglobal_purge_timer(30000)
{
   ...
   // number of Process() executions since FaceTarget() forced on engaged mob
    engaged_heading_tics = 0;
}
...
bool NPC::Process()
{
   ...
   if(!IsMoving()&& !IsMezzed() && !IsStunned())
   {
      if(engaged_heading_tics == RuleI(NPC, EngagedHeadingUpdateTicCount))
      {
         FaceTarget();
         engaged_heading_tics = 0;
      }
      else
         engaged_heading_tics++;
   }
...
void NPC::PickPocket(Client* thief) {
	...
	bool steal_item = (MakeRandomInt(1, 99) < 5 /*|| no_coin*/); //Stop pickpocket item dupe bug
	if (steal_item)
	{
		ItemList::iterator cur,end;
		cur = itemlist.begin();
