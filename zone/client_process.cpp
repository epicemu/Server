...
bool Client::Process() {
	...
	if(Connected() || IsLD())
	{
		...
		if (camp_timer.Check()) {	
			//depop pet on player camp ~ Kegz @ EpicEmu.com
			if(this->GetPet()) {
				 entity_list.RemoveFromHateLists(this->GetPet());
				if (RuleB(Zone, PetZone)) {
					// nothing default to keep pet
				}

				Mob* myPet = this->GetPet();
				if(this->GetPet()->IsCharmed())
				 {   //Null:  charm fades on Invisibility
					myPet->BuffFadeByEffect(SE_Charm);
					//myPet->AddToHateList(this, 100);
					myPet->WipeHateList();
				 }
				//invis break pet
				else if(!this->GetPet()->IsCharmed())
					CastToClient()->SetPet(0);
...
void Client::DoManaRegen() {
	if (GetMana() >= max_mana)
		return;
	// cant regen mana when youre out of beer... Kegz @ EpicEmu.com
	if (Thirsty())
		return;	
