...
//	NPC *npc1, *npc2;
	...
	//pet los added getowner isclient
	if(this->IsEngaged() && this->IsPet() && this->GetOwner()->IsClient() && !this->CheckLosFN(this->GetTarget())){ 
		this->GetTarget()->AddToHateList(this->GetOwner(),500);
		this->hate_list.Wipe();
		return false;
	}
...
bool Mob::CombatRange(Mob* other)
{
	...
	// This section allows modification to the distance at which players must be from a mob in order to attack them.
