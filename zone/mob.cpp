...
bool Mob::HateSummon() {
 ...
 void Mob::FaceTarget(Mob* MobToFace) {
	...
	if(oldheading != newheading) {
		...
		if(moving){
			cout << "Mob::Facetarget() SendPosUpdate" << endl;
			SendPosUpdate();
		}
		else
		{
			cout << "Mob::Facetarget() SendPosition" << endl;
			SendPosition();
		}
	}
	if(IsNPC() && !IsEngaged()) {
...
//Melee/Spell Push
void Mob::Push(Mob *pusher, SkillType st)
 {
 // for now we're only pushing NPCs
 if(!this->IsNPC())
 return;
